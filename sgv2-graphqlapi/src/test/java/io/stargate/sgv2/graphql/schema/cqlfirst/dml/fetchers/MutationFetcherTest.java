...
public class MutationFetcherTest extends DmlTestBase {
  ...
  public void mutationAtomicSingleSelectionTest() {
    assertQuery(
        ...
        "INSERT INTO library.books (title) VALUES (:\"title\")",
        ImmutableMap.of("title", Values.of("a")));
  }
  ...
  public void mutationAtomicMultipleSelectionTest() {
    ...
    assertThat(query1.getCql())
        .isEqualTo("UPDATE library.books SET author = :\"author\" WHERE title = :\"title\"");
    assertThat(toMap(query1.getValues()))
        .hasSize(2)
        .containsEntry("author", Values.of("G.O."))
        .containsEntry("title", Values.of("1984"));

    BatchQuery query2 = batch.getQueries(1);
    assertThat(query2.getCql())
        .isEqualTo("INSERT INTO library.authors (author, title) VALUES (:\"author\", :\"title\")");
    assertThat(toMap(query2.getValues()))
        .hasSize(2)
        .containsEntry("author", Values.of("G.O."))
        .containsEntry("title", Values.of("1984"));

    BatchQuery query3 = batch.getQueries(2);
    assertThat(query3.getCql()).isEqualTo("DELETE FROM library.books WHERE title = :\"title\"");
    assertThat(toMap(query3.getValues()))
        .hasSize(1)
        .containsEntry("title", Values.of("Animal Farm"));
  }
  ...
  private void assertBookAndAuthorInserts(Batch batch) {
    ...
    BatchQuery query1 = batch.getQueries(0);
    assertThat(query1.getCql())
        .isEqualTo("INSERT INTO library.books (title, author) VALUES (:\"title\", :\"author\")");
    assertThat(toMap(query1.getValues()))
        .hasSize(2)
        .containsEntry("author", Values.of("G.O."))
        .containsEntry("title", Values.of("1984"));
  ...
  private Map<String, Value> toMap(QueryOuterClass.Values values) {
