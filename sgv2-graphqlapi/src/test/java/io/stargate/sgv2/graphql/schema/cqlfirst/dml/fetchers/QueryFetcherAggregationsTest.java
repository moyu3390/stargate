  ...
  public static Arguments[] successfulQueries() {
    return new Arguments[] {
      arguments(
          ...
          "SELECT SUM(value) FROM iot.readings WHERE id = :\"id\" GROUP BY year",
          ImmutableMap.of("id", Values.of(1))),
      arguments(
          "readings(value: {id: 1}, groupBy: {month: true, year: true}) { "
              + "values { _float_function(name: \"sum\", args: [\"value\"]) } }",
          "SELECT SUM(value) FROM iot.readings WHERE id = :\"id\" GROUP BY year, month",
          ImmutableMap.of("id", Values.of(1))),
    };
  }
}
